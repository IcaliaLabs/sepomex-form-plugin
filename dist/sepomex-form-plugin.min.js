(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){var axios=require('axios')
const createSepomex=(attributes)=>{const{Sepomex}=require('../models/Sepomex.js');model=new Sepomex();model.id=attributes.id;model.d_codigo=attributes.d_codigo;model.d_asenta=attributes.d_asenta;model.d_tipo_asenta=attributes.d_tipo_asenta;model.d_mnpio=attributes.d_mnpio;model.d_estado=attributes.d_estado;model.d_ciudad=attributes.d_ciudad;model.d_cp=attributes.d_cp;model.c_estado=attributes.c_estado;model.c_oficina=attributes.c_oficina;model.c_cp=attributes.d_cp;model.c_tipo_asenta=attributes.c_tipo_asenta;model.c_mnpio=attributes.c_mnpio;model.id_asenta_cpcons=attributes.id_asenta_cpcons;model.d_zona=attributes.d_zona;model.c_cve_ciudad=attributes.c_cve_ciudad;return model}
const performGetRequest=(params,callback)=>{var BASE_URL="http://sepomex.icalialabs.com/api/v1/zip_codes";var container=new Array;axios.get(BASE_URL,{params:params}).then(function(response){var code_attributes,zip_codes,_i;zip_codes=response.data.zip_codes;for(_i=0;_i<zip_codes.length;_i++){code_attributes=zip_codes[_i];container.push(createSepomex(code_attributes))}
callback(container)}).catch(function(error){console.log(error)})}
exports.performGetRequest=performGetRequest},{"../models/Sepomex.js":3,"axios":5}],2:[function(require,module,exports){(function(global){global.jQuery=$=require('jquery');const{JsModel}=require('javascript-models');const{Sepomex}=require('../models/Sepomex.js');const{SepomexesApi}=require('../models/SepomexesApi');const{performGetRequest}=require('../form-plugin/application_helper.js');var axios=require('axios')(function($){$.fn.autocompleteForm=function(options){$.fn.autocompleteForm.defaults={suburbContainer:"#AddressSuburb",cityContainer:"#AddressCity",stateContainer:"#AddressState",zipCodeContainer:"#AddressZipCode"};var opts=$.extend({},$.fn.autocompleteForm.defaults,options);function completeCity(response){$($.fn.autocompleteForm.defaults.cityContainer).val(response[0].d_mnpio)}
function completeState(response){$($.fn.autocompleteForm.defaults.stateContainer).val(response[0].d_estado)}
function completeZipCode(response){$($.fn.autocompleteForm.defaults.zipCodeContainer).val(response[0].d_codigo)}
$($.fn.autocompleteForm.defaults.zipCodeContainer).focusout(function(){Sepomex.where({zip_code:$($.fn.autocompleteForm.defaults.zipCodeContainer)[0].value,colony:$($.fn.autocompleteForm.defaults.suburbContainer)[0].value,city:$($.fn.autocompleteForm.defaults.cityContainer)[0].value}).then((response)=>{completeCity(response);completeState(response)}).catch((error)=>{console.log("Error in autocompleting zipcode")})});$($.fn.autocompleteForm.defaults.suburbContainer).focusout(function(){Sepomex.where({zip_code:$($.fn.autocompleteForm.defaults.zipCodeContainer)[0].value,colony:$($.fn.autocompleteForm.defaults.suburbContainer)[0].value,city:$($.fn.autocompleteForm.defaults.cityContainer)[0].value}).then((response)=>{completeZipCode(response)
completeCity(response);completeState(response)}).catch((error)=>{console.log("Error in autocompleting suburb")})});$($.fn.autocompleteForm.defaults.cityContainer).focusout(function(){Sepomex.where({zip_code:$($.fn.autocompleteForm.defaults.zipCodeContainer)[0].value,colony:$($.fn.autocompleteForm.defaults.suburbContainer)[0].value,city:$($.fn.autocompleteForm.defaults.cityContainer)[0].value}).then((response)=>{completeState(response)}).catch((error)=>{console.log("Error in autocompleting city")})})}}(jQuery));(function($){$.fn.sepomexForm=function(options){const MEXICO_COORDINATES={lat:25.697094,lng:-100.352872};var opts=$.extend({},$.fn.sepomexForm.defaults,options);map=new google.maps.Map(document.getElementById($.fn.sepomexForm.defaults.mapContainer.substr(1)),{center:MEXICO_COORDINATES,zoom:8});var geocoder=new google.maps.Geocoder();var marker=new google.maps.Marker({map:map,visible:!1});google.maps.event.addListener(map,"click",function(event){geocoder.geocode({'latLng':event.latLng},function(results,status){if(status==google.maps.GeocoderStatus.OK){if(results[0]){document.getElementById($.fn.sepomexForm.defaults.formContainer.substr(1)).reset();let address_components=results[0].address_components
for(var i=0;i<address_components.length;i++){if(address_components[i].types.includes('route')){$($.fn.sepomexForm.defaults.streetContainer).val(address_components[i].long_name)}
else if(address_components[i].types.includes('street_number')){$($.fn.sepomexForm.defaults.numberContainer).val(address_components[i].long_name)}
else if(address_components[i].types.includes('sublocality_level_1')){$($.fn.sepomexForm.defaults.suburbContainer).val(address_components[i].long_name)}
else if(address_components[i].types.includes('locality')){$($.fn.sepomexForm.defaults.cityContainer).val(address_components[i].long_name)}
else if(address_components[i].types.includes('administrative_area_level_2')){$($.fn.sepomexForm.defaults.stateContainer).val(address_components[i].long_name)}
else if(address_components[i].types.includes('country')){$($.fn.sepomexForm.defaults.countryContainer).val(address_components[i].long_name)}
else if(address_components[i].types.includes('postal_code')){$($.fn.sepomexForm.defaults.zipCodeContainer).val(address_components[i].long_name)}}}}});marker.setPosition(event.latLng);marker.setVisible(!0)});function getLatitudeLongitudeAndSetInMap(callback){let address=getAddressFromForm();geocoder.geocode({'address':address},function(results,status){if(status==google.maps.GeocoderStatus.OK){callback(results[0].geometry.location)}})}
function getAddressFromForm(){var address="";$($.fn.sepomexForm.defaults.formContainer+' :input').each(function(){if(this.value){address+=(" "+this.value)}});return address}
$($.fn.sepomexForm.defaults.formContainer).focusout(function(){getLatitudeLongitudeAndSetInMap(function(latLng){map.setCenter(latLng);marker.setVisible(!0);marker.setPosition(latLng);map.setZoom(15)})})}
$.fn.sepomexForm.defaults={formContainer:"#AddressForm",mapContainer:"#MapWrapper",numberContainer:"#AddressNumber",streetContainer:"#AddressStreet",suburbContainer:"#AddressSuburb",cityContainer:"#AddressCity",stateContainer:"#AddressState",countryContainer:"#AddressCountry",zipCodeContainer:"#AddressZipCode"}}(jQuery))}).call(this,typeof global!=="undefined"?global:typeof self!=="undefined"?self:typeof window!=="undefined"?window:{})},{"../form-plugin/application_helper.js":1,"../models/Sepomex.js":3,"../models/SepomexesApi":4,"axios":5,"javascript-models":31,"jquery":39}],3:[function(require,module,exports){const{JsModel}=require('javascript-models');const{SepomexesApi}=require('./SepomexesApi');class Sepomex extends JsModel{static ATTRIBUTES(){return["id","d_codigo","d_asenta","d_tipo_asenta","d_mnpio","d_estado","d_ciudad","d_cp","c_estado","c_oficina","c_cp","c_tipo_asenta","c_mnpio","id_asenta_cpcons","d_zona","c_cve_ciudad"]}
static API_CLASS(){return SepomexesApi}}
exports.Sepomex=Sepomex},{"./SepomexesApi":4,"javascript-models":31}],4:[function(require,module,exports){const{performGetRequest}=require('../form-plugin/application_helper.js');const{Sepomex}=require('../models/Sepomex.js');class SepomexesApi{static postCreate(params){}
...
return promise};utils.forEach(['delete','get','head','options'],function forEachMethodNoData(method){Axios.prototype[method]=function(url,config){return this.request(utils.merge(config||{},{method:method,url:url}))}});utils.forEach(['post','put','patch'],function forEachMethodWithData(method){Axios.prototype[method]=function(url,data,config){return this.request(utils.merge(config||{},{method:method,url:url,data:data}))}});module.exports=Axios},{"./../defaults":18,"./../utils":29,"./InterceptorManager":12,"./dispatchRequest":14}],12:[function(require,module,exports){'use strict';var utils=require('./../utils');function InterceptorManager(){this.handlers=[]}
InterceptorManager.prototype.use=function use(fulfilled,rejected){this.handlers.push({fulfilled:fulfilled,rejected:rejected});return this.handlers.length-1};InterceptorManager.prototype.eject=function eject(id){if(this.handlers[id]){this.handlers[id]=null}};InterceptorManager.prototype.forEach=function forEach(fn){utils.forEach(this.handlers,function forEachHandler(h){if(h!==null){fn(h)}})};module.exports=InterceptorManager},{"./../utils":29}],13:[function(require,module,exports){'use strict';var enhanceError=require('./enhanceError');module.exports=function createError(message,config,code,request,response){var error=new Error(message);return enhanceError(error,config,code,request,response)}},{"./enhanceError":15}],14:[function(require,module,exports){'use strict';var utils=require('./../utils');var transformData=require('./transformData');var isCancel=require('../cancel/isCancel');var defaults=require('../defaults');var isAbsoluteURL=require('./../helpers/isAbsoluteURL');var combineURLs=require('./../helpers/combineURLs');function throwIfCancellationRequested(config){if(config.cancelToken){config.cancelToken.throwIfRequested()}}
...
return Promise.reject(reason)})}},{"../cancel/isCancel":10,"../defaults":18,"./../helpers/combineURLs":22,"./../helpers/isAbsoluteURL":24,"./../utils":29,"./transformData":17}],15:[function(require,module,exports){'use strict';module.exports=function enhanceError(error,config,code,request,response){error.config=config;if(code){error.code=code}
error.request=request;error.response=response;return error}},{}],16:[function(require,module,exports){'use strict';var createError=require('./createError');module.exports=function settle(resolve,reject,response){var validateStatus=response.config.validateStatus;if(!response.status||!validateStatus||validateStatus(response.status)){resolve(response)}else{reject(createError('Request failed with status code '+response.status,response.config,null,response.request,response))}}},{"./createError":13}],17:[function(require,module,exports){'use strict';var utils=require('./../utils');module.exports=function transformData(data,headers,fns){utils.forEach(fns,function transform(fn){data=fn(data,headers)});return data}},{"./../utils":29}],18:[function(require,module,exports){(function(process){'use strict';var utils=require('./utils');var normalizeHeaderName=require('./helpers/normalizeHeaderName');var DEFAULT_CONTENT_TYPE={'Content-Type':'application/x-www-form-urlencoded'};function setContentTypeIfUnset(headers,value){if(!utils.isUndefined(headers)&&utils.isUndefined(headers['Content-Type'])){headers['Content-Type']=value}}
...
return data}],timeout:0,xsrfCookieName:'XSRF-TOKEN',xsrfHeaderName:'X-XSRF-TOKEN',maxContentLength:-1,validateStatus:function validateStatus(status){return status>=200&&status<300}};defaults.headers={common:{'Accept':'application/json, text/plain, */*'}};utils.forEach(['delete','get','head'],function forEachMethodNoData(method){defaults.headers[method]={}});utils.forEach(['post','put','patch'],function forEachMethodWithData(method){defaults.headers[method]=utils.merge(DEFAULT_CONTENT_TYPE)});module.exports=defaults}).call(this,require('_process'))},{"./adapters/http":6,"./adapters/xhr":6,"./helpers/normalizeHeaderName":26,"./utils":29,"_process":108}],19:[function(require,module,exports){'use strict';module.exports=function bind(fn,thisArg){return function wrap(){var args=new Array(arguments.length);for(var i=0;i<args.length;i++){args[i]=arguments[i]}
return fn.apply(thisArg,args)}}},{}],20:[function(require,module,exports){'use strict';var chars='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=';function E(){this.message='String contains an invalid character'}
...
return output}
module.exports=btoa},{}],21:[function(require,module,exports){'use strict';var utils=require('./../utils');function encode(val){return encodeURIComponent(val).replace(/%40/gi,'@').replace(/%3A/gi,':').replace(/%24/g,'$').replace(/%2C/gi,',').replace(/%20/g,'+').replace(/%5B/gi,'[').replace(/%5D/gi,']')}
...
module.exports={isArray:isArray,isArrayBuffer:isArrayBuffer,isBuffer:isBuffer,isFormData:isFormData,isArrayBufferView:isArrayBufferView,isString:isString,isNumber:isNumber,isObject:isObject,isUndefined:isUndefined,isDate:isDate,isFile:isFile,isBlob:isBlob,isFunction:isFunction,isStream:isStream,isURLSearchParams:isURLSearchParams,isStandardBrowserEnv:isStandardBrowserEnv,forEach:forEach,merge:merge,extend:extend,trim:trim}},{"./helpers/bind":19,"is-buffer":30}],30:[function(require,module,exports){module.exports=function(obj){return obj!=null&&(isBuffer(obj)||isSlowBuffer(obj)||!!obj._isBuffer)}
function isBuffer(obj){return!!obj.constructor&&typeof obj.constructor.isBuffer==='function'&&obj.constructor.isBuffer(obj)}
function isSlowBuffer(obj){return typeof obj.readFloatLE==='function'&&typeof obj.slice==='function'&&isBuffer(obj.slice(0,0))}},{}],31:[function(require,module,exports){const{JsModel}=require('./jsmodels');exports.JsModel=JsModel},{"./jsmodels":35}],32:[function(require,module,exports){const{errorResponse}=require('./errorResponse');class ApiClassValidator{static ERRORS(params={apiClass:'API_CLASS',endpoint:'Endpoint'}){const{endpoint}=params;return{default_error:errorResponse({error:'something went wrong with the implementation'}),endpoint_not_implemented:errorResponse({error:`${endpoint} not implemented in API`}),endpoint_is_not_function:errorResponse({error:`Api endpoint ${endpoint} is not a function`}),endpoint_invalid_return:errorResponse({error:`${endpoint} did not returned a Promise`}),api_class_not_implemented:errorResponse({error:'API class not specified'}),api_not_a_class:errorResponse({error:"API class is implemented but it's not a class"}),}}
static isValidApiClass(apiClass){return(typeof apiClass==='function'||typeof apiClass==='object')&&apiClass!==null}
static apiClassImplementsEndpoint(apiClass,endpoint){return typeof apiClass[endpoint]==='function'&&apiClass[endpoint]!==undefined&&apiClass[endpoint]!==null}
static invalidApiClassError(apiClass,endpoint){const errors=this.ERRORS({apiClass,endpoint});let error=errors.default_error;if(this.isValidApiClass(apiClass)){const endpointFunction=apiClass[endpoint];if(!endpointFunction||endpointFunction===null){error=errors.endpoint_not_implemented}else if(typeof endpointFunction!=='function'){error=errors.endpoint_is_not_function}}else if(apiClass===null){error=errors.api_class_not_implemented}else if(typeof apiClass!=='function'){error=errors.api_not_a_class}
return error}
static invalidApiClassPromise(apiClass,endpoint){return Promise.reject(this.invalidApiClassError(apiClass,endpoint))}
static endpointInvalidReturnPromise(endpoint){const errors=this.ERRORS({endpoint});return Promise.reject(errors.endpoint_invalid_return)}}
exports.ApiClassValidator=ApiClassValidator},{"./errorResponse":34}],33:[function(require,module,exports){const{ApiClassValidator}=require('./ApiClassValidator');const{isPromise}=require('../utils/isPromise');const{Validator}=require('../validator/');class JsModelMethodsHandler{static handleApiCallWithValidations(args){const apiClass=args.apiClass;const endpoint=args.endpoint;const params=args.params;const customValidator=args.customValidator;const validations=args.validations;const modelClass=args.modelClass;if(ApiClassValidator.isValidApiClass(apiClass)&&ApiClassValidator.apiClassImplementsEndpoint(apiClass,endpoint)){let validatorResponse;if(customValidator){validatorResponse=Validator.validate(params,validations,customValidator)}else{validatorResponse=Validator.validate(params,validations)}
if(validatorResponse.valid){const promise=apiClass[endpoint](params);if(isPromise(promise)){return promise}
return ApiClassValidator.endpointInvalidReturnPromise(endpoint)}
return modelClass.invalidModelPromise(params,validatorResponse)}
return ApiClassValidator.invalidApiClassPromise(apiClass,endpoint)}
static handleApiCallWithoutValidations(args){const apiClass=args.apiClass;const endpoint=args.endpoint;const params=args.params;if(ApiClassValidator.isValidApiClass(apiClass)&&ApiClassValidator.apiClassImplementsEndpoint(apiClass,endpoint)){const promise=apiClass[endpoint](params);if(isPromise(promise)){return promise}
return ApiClassValidator.endpointInvalidReturnPromise(endpoint)}
return ApiClassValidator.invalidApiClassPromise(apiClass,endpoint)}}
exports.JsModelMethodsHandler=JsModelMethodsHandler},{"../utils/isPromise":36,"../validator/":37,"./ApiClassValidator":32}],34:[function(require,module,exports){const errorResponse=(data)=>{const response={status:0,ok:!1,type:'error'};response.data=data;return response};exports.errorResponse=errorResponse},{}],35:[function(require,module,exports){const{JsModelMethodsHandler}=require('./JsModelMethodsHandler');const{Validator}=require('../validator/');const{errorResponse}=require('./errorResponse');class JsModel{static ATTRIBUTES(){return[]}
static VALIDATIONS(){return{}}
static API_CLASS(){return null}
static CUSTOM_VALIDATOR(){return null}
constructor(modelParams={}){this.constructor.ATTRIBUTES().forEach((attribute)=>{if(attribute in modelParams){this[attribute]=modelParams[attribute]}else{this[attribute]=null}});this.errors={}}
static create(params){const apiClass=this.API_CLASS();const endpoint='postCreate';const customValidator=this.CUSTOM_VALIDATOR();const validations=this.VALIDATIONS();const modelClass=this;const args={apiClass,endpoint,customValidator,validations,params,modelClass,};return JsModelMethodsHandler.handleApiCallWithValidations(args)}
save(){const apiClass=this.constructor.API_CLASS();const endpoint='postCreate';const customValidator=this.constructor.CUSTOM_VALIDATOR();const validations=this.constructor.VALIDATIONS();const params=this.modelParams();const modelClass=this.constructor;const args={apiClass,endpoint,customValidator,validations,params,modelClass,};return JsModelMethodsHandler.handleApiCallWithValidations(args)}
static find(queryParams){const apiClass=this.API_CLASS();const endpoint='getFind';const params=queryParams;const args={apiClass,endpoint,params,};return JsModelMethodsHandler.handleApiCallWithoutValidations(args)}
static all(queryParams={}){const apiClass=this.API_CLASS();const endpoint='getAll';const params=queryParams;const args={apiClass,endpoint,params,};return JsModelMethodsHandler.handleApiCallWithoutValidations(args)}
static first(queryParams={}){const apiClass=this.API_CLASS();const endpoint='getFirst';const params=queryParams;const args={apiClass,endpoint,params,};return JsModelMethodsHandler.handleApiCallWithoutValidations(args)}
static findBy(queryParams){const apiClass=this.API_CLASS();const endpoint='getFindBy';const params=queryParams;const args={apiClass,endpoint,params,};return JsModelMethodsHandler.handleApiCallWithoutValidations(args)}
static where(queryParams){const apiClass=this.API_CLASS();const endpoint='getWhere';const params=queryParams;const args={apiClass,endpoint,params,};return JsModelMethodsHandler.handleApiCallWithoutValidations(args)}
static updateAll(params){const apiClass=this.API_CLASS();const endpoint='putUpdate';const customValidator=this.CUSTOM_VALIDATOR();const validations=this.VALIDATIONS();const modelClass=this;const args={apiClass,endpoint,customValidator,validations,params,modelClass,};return JsModelMethodsHandler.handleApiCallWithValidations(args)}
update(){const apiClass=this.constructor.API_CLASS();const endpoint='putUpdate';const customValidator=this.constructor.CUSTOM_VALIDATOR();const validations=this.constructor.VALIDATIONS();const params=this.modelParams();const modelClass=this.constructor;const args={apiClass,endpoint,customValidator,validations,params,modelClass,};return JsModelMethodsHandler.handleApiCallWithValidations(args)}
destroy(){const apiClass=this.constructor.API_CLASS();const endpoint='deleteDestroy';const params=this.modelParams();const modelClass=this.constructor;const args={apiClass,endpoint,params,modelClass,};return JsModelMethodsHandler.handleApiCallWithoutValidations(args)}
isValid(){const customValidator=this.constructor.CUSTOM_VALIDATOR();const attributesValues=this.modelParams();const validations=this.constructor.VALIDATIONS();let validatorResponse;if(customValidator){validatorResponse=Validator.validate(attributesValues,validations,customValidator)}else{validatorResponse=Validator.validate(attributesValues,validations)}
if(!validatorResponse.valid){this.errors=validatorResponse.errors}
return validatorResponse.valid}
isInvalid(){return!this.isValid()}
modelParams(){const modelParams={};this.constructor.ATTRIBUTES().forEach((attribute)=>{modelParams[attribute]=this[attribute]});return modelParams}
static invalidModelPromise(params,validatorResponse){return Promise.reject(errorResponse({error:'Model is not valid',model:this.modelInstanceWithErrors(params,validatorResponse),}))}
static modelInstanceWithErrors(attributes,validatorResponse={errors:{}}){const modelInstance=new this(attributes);modelInstance.errors=validatorResponse.errors;return modelInstance}}
exports.JsModel=JsModel},{"../validator/":37,"./JsModelMethodsHandler":33,"./errorResponse":34}],36:[function(require,module,exports){function isPromise(promise){return typeof(promise)==='object'&&typeof(promise.then)==='function'&&typeof(promise.catch)==='function'}
exports.isPromise=isPromise},{}],37:[function(require,module,exports){const{ValidatorDictionary}=require('./validations_dictionaries/ValidatorDictionary.js');class Validator{static VALIDATIONS_DICTIONARY(){return Object.assign({},ValidatorDictionary.DICTIONARY())}
constructor(values={},validations={},CustomValidator=null){this.values=values;this.validations=validations;this.validationsDictionary=Validator.VALIDATIONS_DICTIONARY();if(CustomValidator){this.validationsDictionary=Validator.mergeCustomValidatorDictionary(this.validationsDictionary,CustomValidator.VALIDATIONS_DICTIONARY())}}
static validate(values={},validations={},customValidator=null){const validator=new Validator(values,validations,customValidator);return validator.runValidations()}
static mergeCustomValidatorDictionary(validationsDictionary,CustomValidator){return Object.assign({},validationsDictionary,CustomValidator)}
runValidations(){let valid=!0;const errors={};const attributesKeys=Object.keys(this.validations);attributesKeys.forEach((attributeKey)=>{const attributeValidations=this.validations[attributeKey];const attributeResults=this.runAttributeValidations(attributeKey,attributeValidations);if(valid&&!attributeResults.valid){valid=!1}
Object.assign(errors,attributeResults.errors)});return{valid,errors}}
runAttributeValidations(attributeKey,attributeValidations){let validAttribute=!0;let errors={};const attributeValidationsKeys=Object.keys(attributeValidations);attributeValidationsKeys.forEach((attributeValidationKey)=>{const value=this.values[attributeKey];const params=attributeValidations[attributeValidationKey];const currentAttributeValidation=this.validationsDictionary[attributeValidationKey];const validationResult=this.runValidation(attributeValidationKey,currentAttributeValidation,value,params);if(!validationResult.passed){validAttribute=!1;errors=this.addAttributeError(errors,attributeKey,validationResult.error)}});return{valid:validAttribute,errors}}
runValidation(validationKey,validation,value,params){let passed=!0;let error;if(!validation){error=`Validation ${validationKey} doesn't exist.`;passed=!1;return{passed,error}}else if(!Validator.isFunction(validation.validate)){error=`${validationKey} is not a function.`;passed=!1;return{passed,error}}
passed=validation.validate(value,params);if(Validator.isBoolean(passed)){error=`Validation ${validationKey} didn't return a boolean.`;passed=!1}else if(!passed&&!validation.error){error=`Must pass the ${validation.validate.name} validation.`}else if(!passed){error=validation.error}
return{passed,error}}
static isFunction(value){return value&&typeof(value)==='function'}
static isBoolean(value){return typeof(value)!=='boolean'}
addAttributeError(errors,attributeKey,validationError){if(errors[attributeKey]){errors[attributeKey].push(validationError)}else{errors[attributeKey]=[validationError]}
return errors}}
exports.Validator=Validator},{"./validations_dictionaries/ValidatorDictionary.js":38}],38:[function(require,module,exports){const validator=require('validator');class ValidatorDictionary{static DICTIONARY(){return{email:{validate:this.isEmail,error:'The value must be an email.'},alpha:{validate:this.isAlpha,error:'The value must be alpha.'},alphanumeric:{validate:this.isAlphanumeric,error:'The value must be alphanumeric.'},boolen:{validate:this.isBoolean,error:'The value must be boolean.'},creditCard:{validate:this.isCreditCard,error:'The value must be a credit card.'},decimal:{validate:this.isDecimal,error:'The value must be a decimal.'},empty:{validate:this.isEmpty,error:'The value must be empty.'},empty:{validate:this.isNotEmpty,error:'The value must be empty.'},float:{validate:this.isFloat,error:'The value must be a float number.'},hexColor:{validate:this.isHexColor,error:'The value must be a Hex color.'},hexadecimal:{validate:this.isHexadecimal,error:'The value must bea a hexadecimal number.'},integer:{validate:this.isInt,error:'The value must be an integer number.'},json:{validate:this.isJSON,error:'The value must be in JSON format.'},lowercase:{validate:this.isLowercase,error:'The value must be in lowercase.'},length:{validate:this.isLength,error:"The length doesn't comfort to the specified values."},numeric:{validate:this.isNumeric,error:'The value must be numeric.'},uppercase:{validate:this.isUppercase,error:'The value must be in lowercase.'},coordinates:{validate:this.isLatLong,error:'The value must be a latitud-longitude coordinate.'},MAC:{validate:this.isMACAddress,error:'The value must be a MAC address.'},}}
static isEmail(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isEmail(value,args)}
static isAlpha(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isAlpha(value,args)}
static isAlphanumeric(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isAlphanumeric(value,args)}
static isBoolean(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isBoolean(value,args)}
static isCreditCard(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isCreditCard(value,args)}
static isDecimal(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isDecimal(value,args)}
static isEmpty(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isEmpty(value,args)}
static isNotEmpty(value,args){value=ValidatorDictionary.convertToString(value)
return!validator.isEmpty(value,args)}
static isFloat(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isFloat(value,args)}
static isHexColor(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isHexColor(value,args)}
static isHexadecimal(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isHexadecimal(value,args)}
static isInt(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isInt(value,args)}
static isJSON(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isJSON(value,args)}
static isLowercase(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isLowercase(value,args)}
static isLength(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isLength(value,args)}
static isNumeric(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isNumeric(value,args)}
static isUppercase(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isUppercase(value,args)}
static isLatLong(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isLatLong(value,args)}
static isMACAddress(value,args){value=ValidatorDictionary.convertToString(value)
return validator.isMACAddress(value,args)}
static convertToString(value){if(typeof value!=='String'){value=String(value)}
return value}}
exports.ValidatorDictionary=ValidatorDictionary},{"validator":40}],39:[function(require,module,exports){(function(global,factory){"use strict";if(typeof module==="object"&&typeof module.exports==="object"){module.exports=global.document?factory(global,!0):function(w){if(!w.document){throw new Error("jQuery requires a window with a document")}
...
module.exports=exports['default']},{"./util/assertString":104}],54:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=isCreditCard;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
var creditCard=/^(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|(222[1-9]|22[3-9][0-9]|2[3-6][0-9]{2}|27[01][0-9]|2720)[0-9]{12}|6(?:011|5[0-9][0-9])[0-9]{12}|3[47][0-9]{13}|3(?:0[0-5]|[68][0-9])[0-9]{11}|(?:2131|1800|35\d{3})\d{11}|62[0-9]{14})$/;function isCreditCard(str){(0,_assertString2.default)(str);var sanitized=str.replace(/[- ]+/g,'');if(!creditCard.test(sanitized)){return!1}
...
var pattern=options.allow_utf8_local_part?emailUserUtf8Part:emailUserPart;var user_parts=user.split('.');for(var i=0;i<user_parts.length;i++){if(!pattern.test(user_parts[i])){return!1}}
return!0}
module.exports=exports['default']},{"./isByteLength":53,"./isFQDN":61,"./util/assertString":104,"./util/merge":105}],60:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=isEmpty;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
function isEmpty(str){(0,_assertString2.default)(str);return str.length===0}
module.exports=exports['default']},{"./util/assertString":104}],61:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=isFDQN;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);var _merge=require('./util/merge');var _merge2=_interopRequireDefault(_merge);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
var default_fqdn_options={require_tld:!0,allow_underscores:!1,allow_trailing_dot:!1};function isFDQN(str,options){(0,_assertString2.default)(str);options=(0,_merge2.default)(options,default_fqdn_options);if(options.allow_trailing_dot&&str[str.length-1]==='.'){str=str.substring(0,str.length-1)}
...
return!1}
module.exports=exports['default']},{"./util/assertString":104}],69:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=isISBN;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
var isbn10Maybe=/^(?:[0-9]{9}X|[0-9]{10})$/;var isbn13Maybe=/^(?:[0-9]{13})$/;var factor=[1,3];function isISBN(str){var version=arguments.length>1&&arguments[1]!==undefined?arguments[1]:'';(0,_assertString2.default)(str);version=String(version);if(!version){return isISBN(str,10)||isISBN(str,13)}
var sanitized=str.replace(/[\s-]+/g,'');var checksum=0;var i=void 0;if(version==='10'){if(!isbn10Maybe.test(sanitized)){return!1}
for(i=0;i<9;i++){checksum+=(i+1)*sanitized.charAt(i)}
if(sanitized.charAt(9)==='X'){checksum+=10*10}else{checksum+=10*sanitized.charAt(9)}
if(checksum%11===0){return!!sanitized}}else if(version==='13'){if(!isbn13Maybe.test(sanitized)){return!1}
for(i=0;i<12;i++){checksum+=factor[i%2]*sanitized.charAt(i)}
if(sanitized.charAt(12)-(10-checksum%10)%10===0){return!!sanitized}}
return!1}
module.exports=exports['default']},{"./util/assertString":104}],70:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=isISIN;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
var isin=/^[A-Z]{2}[0-9A-Z]{9}[0-9]$/;function isISIN(str){(0,_assertString2.default)(str);if(!isin.test(str)){return!1}
var checksumStr=str.replace(/[A-Z]/g,function(character){return parseInt(character,36)});var sum=0;var digit=void 0;var tmpNum=void 0;var shouldDouble=!0;for(var i=checksumStr.length-2;i>=0;i--){digit=checksumStr.substring(i,i+1);tmpNum=parseInt(digit,10);if(shouldDouble){tmpNum*=2;if(tmpNum>=10){sum+=tmpNum+1}else{sum+=tmpNum}}else{sum+=tmpNum}
shouldDouble=!shouldDouble}
return parseInt(str.substr(str.length-1),10)===(10000-sum)%10}
module.exports=exports['default']},{"./util/assertString":104}],71:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=isISO8601;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
var iso8601=/^([\+-]?\d{4}(?!\d{2}\b))((-?)((0[1-9]|1[0-2])(\3([12]\d|0[1-9]|3[01]))?|W([0-4]\d|5[0-2])(-?[1-7])?|(00[1-9]|0[1-9]\d|[12]\d{2}|3([0-5]\d|6[1-6])))([T\s]((([01]\d|2[0-3])((:?)[0-5]\d)?|24:?00)([\.,]\d+(?!:))?)?(\17[0-5]\d([\.,]\d+)?)?([zZ]|([\+-])([01]\d|2[0-3]):?([0-5]\d)?)?)?)?$/;function isISO8601(str){(0,_assertString2.default)(str);return iso8601.test(str)}
module.exports=exports['default']},{"./util/assertString":104}],72:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=isISRC;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
var isrc=/^[A-Z]{2}[0-9A-Z]{3}\d{2}\d{5}$/;function isISRC(str){(0,_assertString2.default)(str);return isrc.test(str)}
module.exports=exports['default']},{"./util/assertString":104}],73:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=isISSN;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
...
var issnDigits=str.replace('-','');var position=8;var checksum=0;var _iteratorNormalCompletion=!0;var _didIteratorError=!1;var _iteratorError=undefined;try{for(var _iterator=issnDigits[Symbol.iterator](),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=!0){var digit=_step.value;var digitValue=digit.toUpperCase()==='X'?10:+digit;checksum+=digitValue*position;--position}}catch(err){_didIteratorError=!0;_iteratorError=err}finally{try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return()}}finally{if(_didIteratorError){throw _iteratorError}}}
return checksum%11===0}
module.exports=exports['default']},{"./util/assertString":104}],74:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});var _typeof=typeof Symbol==="function"&&typeof Symbol.iterator==="symbol"?function(obj){return typeof obj}:function(obj){return obj&&typeof Symbol==="function"&&obj.constructor===Symbol&&obj!==Symbol.prototype?"symbol":typeof obj};exports.default=isIn;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);var _toString=require('./util/toString');var _toString2=_interopRequireDefault(_toString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
function isIn(str,options){(0,_assertString2.default)(str);var i=void 0;if(Object.prototype.toString.call(options)==='[object Array]'){var array=[];for(i in options){if({}.hasOwnProperty.call(options,i)){array[i]=(0,_toString2.default)(options[i])}}
...
module.exports=exports['default']},{"./util/assertString":104}],94:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=matches;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
function matches(str,pattern,modifiers){(0,_assertString2.default)(str);if(Object.prototype.toString.call(pattern)!=='[object RegExp]'){pattern=new RegExp(pattern,modifiers)}
return pattern.test(str)}
module.exports=exports['default']},{"./util/assertString":104}],95:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=normalizeEmail;var _isEmail=require('./isEmail');var _isEmail2=_interopRequireDefault(_isEmail);var _merge=require('./util/merge');var _merge2=_interopRequireDefault(_merge);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
var default_normalize_email_options={all_lowercase:!0,gmail_lowercase:!0,gmail_remove_dots:!0,gmail_remove_subaddress:!0,gmail_convert_googlemaildotcom:!0,outlookdotcom_lowercase:!0,outlookdotcom_remove_subaddress:!0,yahoo_lowercase:!0,yahoo_remove_subaddress:!0,icloud_lowercase:!0,icloud_remove_subaddress:!0};var icloud_domains=['icloud.com','me.com'];var outlookdotcom_domains=['hotmail.at','hotmail.be','hotmail.ca','hotmail.cl','hotmail.co.il','hotmail.co.nz','hotmail.co.th','hotmail.co.uk','hotmail.com','hotmail.com.ar','hotmail.com.au','hotmail.com.br','hotmail.com.gr','hotmail.com.mx','hotmail.com.pe','hotmail.com.tr','hotmail.com.vn','hotmail.cz','hotmail.de','hotmail.dk','hotmail.es','hotmail.fr','hotmail.hu','hotmail.id','hotmail.ie','hotmail.in','hotmail.it','hotmail.jp','hotmail.kr','hotmail.lv','hotmail.my','hotmail.ph','hotmail.pt','hotmail.sa','hotmail.sg','hotmail.sk','live.be','live.co.uk','live.com','live.com.ar','live.com.mx','live.de','live.es','live.eu','live.fr','live.it','live.nl','msn.com','outlook.at','outlook.be','outlook.cl','outlook.co.il','outlook.co.nz','outlook.co.th','outlook.com','outlook.com.ar','outlook.com.au','outlook.com.br','outlook.com.gr','outlook.com.pe','outlook.com.tr','outlook.com.vn','outlook.cz','outlook.de','outlook.dk','outlook.es','outlook.fr','outlook.hu','outlook.id','outlook.ie','outlook.in','outlook.it','outlook.jp','outlook.kr','outlook.lv','outlook.my','outlook.ph','outlook.pt','outlook.sa','outlook.sg','outlook.sk','passport.com'];var yahoo_domains=['rocketmail.com','yahoo.ca','yahoo.co.uk','yahoo.com','yahoo.de','yahoo.fr','yahoo.in','yahoo.it','ymail.com'];function normalizeEmail(email,options){options=(0,_merge2.default)(options,default_normalize_email_options);if(!(0,_isEmail2.default)(email)){return!1}
var raw_parts=email.split('@');var domain=raw_parts.pop();var user=raw_parts.join('@');var parts=[user,domain];parts[1]=parts[1].toLowerCase();if(parts[1]==='gmail.com'||parts[1]==='googlemail.com'){if(options.gmail_remove_subaddress){parts[0]=parts[0].split('+')[0]}
if(options.gmail_remove_dots){parts[0]=parts[0].replace(/\./g,'')}
if(!parts[0].length){return!1}
if(options.all_lowercase||options.gmail_lowercase){parts[0]=parts[0].toLowerCase()}
parts[1]=options.gmail_convert_googlemaildotcom?'gmail.com':parts[1]}else if(~icloud_domains.indexOf(parts[1])){if(options.icloud_remove_subaddress){parts[0]=parts[0].split('+')[0]}
if(!parts[0].length){return!1}
if(options.all_lowercase||options.icloud_lowercase){parts[0]=parts[0].toLowerCase()}}else if(~outlookdotcom_domains.indexOf(parts[1])){if(options.outlookdotcom_remove_subaddress){parts[0]=parts[0].split('+')[0]}
if(!parts[0].length){return!1}
if(options.all_lowercase||options.outlookdotcom_lowercase){parts[0]=parts[0].toLowerCase()}}else if(~yahoo_domains.indexOf(parts[1])){if(options.yahoo_remove_subaddress){var components=parts[0].split('-');parts[0]=components.length>1?components.slice(0,-1).join('-'):components[0]}
if(!parts[0].length){return!1}
if(options.all_lowercase||options.yahoo_lowercase){parts[0]=parts[0].toLowerCase()}}else if(options.all_lowercase){parts[0]=parts[0].toLowerCase()}
return parts.join('@')}
module.exports=exports['default']},{"./isEmail":59,"./util/merge":105}],96:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=rtrim;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
function rtrim(str,chars){(0,_assertString2.default)(str);var pattern=chars?new RegExp('['+chars+']'):/\s/;var idx=str.length-1;while(idx>=0&&pattern.test(str[idx])){idx--}
return idx<str.length?str.substr(0,idx+1):str}
module.exports=exports['default']},{"./util/assertString":104}],97:[function(require,module,exports){'use strict';Object.defineProperty(exports,"__esModule",{value:!0});exports.default=stripLow;var _assertString=require('./util/assertString');var _assertString2=_interopRequireDefault(_assertString);var _blacklist=require('./blacklist');var _blacklist2=_interopRequireDefault(_blacklist);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}
...
process.binding=function(name){throw new Error('process.binding is not supported')};process.cwd=function(){return'/'};process.chdir=function(dir){throw new Error('process.chdir is not supported')};process.umask=function(){return 0}},{}]},{},[2])
